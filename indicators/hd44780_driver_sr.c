...
HD44780_Result hd44780_write_bits(HD44780_Interface *interface, uint8_t value);



HD44780_Result hd44780_driver_init(HD44780_Interface *interface, HD44780_Mode addr_mode)
{
	HD44780_Driver *driver = (HD44780_Driver*)interface;
...
static HD44780_Result hd44780_driver_send(HD44780_Interface *interface, uint8_t value, HD44780_PinState rs_mode)
{
	...
	if (rs_mode == HD44780_PINSTATE_HIGH)
		driver->out_buf |= driver->pin_mask[HD44780_PIN_RS];
	else
		driver->out_buf &= ~(driver->pin_mask[HD44780_PIN_RS]);
...
HD44780_Result hd44780_write_bits(HD44780_Interface *interface, uint8_t value)
{
  ...
  for (uint8_t i = 0; i < driver->dp_amount; ++i)
  	{
  		if ((value >> i) & 0x01)
  		driver->out_buf |= driver->pin_mask[driver->dp_first + i];
...
const HD44780_Interface HD44780_INTERFACE =
{
	hd44780_driver_init,
	hd44780_driver_send
};
